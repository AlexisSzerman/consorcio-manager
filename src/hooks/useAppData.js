...
function coincideConReconciliacion(m) {
  return m.categoria === "proveedor" && m.tipo === "egreso";
}
...
export function useAppData() {
  ...
  const [
    ...
    setLibroDiarioMovimientosPorPeriodo,
  ] = useState({});
  ...
  const ultimaActualizacionGlobal = useMemo(() => {
    ...
    return movimientos.reduce((max, m) => {
      ...
      return !max || m.estado_actualizado_en > max
        ? m.estado_actualizado_en
        : max;
    }, null);
  }, [movimientos]);
  ...
  const addUnidad = useCallback(
    async (
      ...
      setConsorcios((prev) =>
        prev.map((c) =>
          c.id === consorcioId
            ? { ...c, unidades: [...c.unidades, nuevaUnidad] }
            : c,
        ),
      );
        ...
        c.id === consorcioId
          ? {
              ...
              unidades: c.unidades.map((u) =>
                u.id === unidadId ? unidadActualizada : u,
              ),
            }
          : c,
      ),
  ...
  // ---------- Libro Diario ----------
  const addPeriodoLibroDiario = useCallback(
    async (consorcioId, periodo, cuenta, banco) => {
      const { data, error } = await supabase
        .from("libro_diario_periodos")
        .insert({
          consorcio_id: consorcioId,
  ...
  const updatePeriodoLibroDiario = useCallback(async (periodoId, campos) => {
    const { data, error } = await supabase
      ...
      .single();
    if (error) throw error;
    setLibroDiarioPeriodos((prev) =>
      prev.map((p) => (p.id === periodoId ? data : p)),
    );
    return data;
  }, []);
  ...
  const addMovimientoLibroDiario = useCallback(async (periodoId, campos) => {
    ...
    if (coincideConReconciliacion(data)) {
      setLibroDiarioParaReconciliar((prev) => [...prev, data]);
    }
    return data;
  }, []);

  const updateMovimientoLibroDiario = useCallback(
    async (periodoId, movId, campos) => {
      const { data, error } = await supabase
        .from("libro_diario_movimientos")
        .update(campos)
        .eq("id", movId)
      ...
      setLibroDiarioMovimientosPorPeriodo((prev) => ({
        ...prev,
        [periodoId]: (prev[periodoId] || []).map((m) =>
          m.id === movId ? data : m,
        ),
      }));
      setLibroDiarioParaReconciliar((prev) => {
        const sinEste = prev.filter((m) => m.id !== movId);
        return coincideConReconciliacion(data) ? [...sinEste, data] : sinEste;
      });
      return data;
    },
    [],
  ...
  const deleteMovimientoLibroDiario = useCallback(async (periodoId, movId) => {
    ...
    setLibroDiarioParaReconciliar((prev) => prev.filter((m) => m.id !== movId));
  }, []);

  const addMovimientosLibroDiarioBulk = useCallback(
    async (periodoId, movimientosArray) => {
      if (movimientosArray.length === 0) return [];
      const payload = movimientosArray.map((m) => ({
      ...
      const nuevosParaReconciliar = data.filter(coincideConReconciliacion);
      if (nuevosParaReconciliar.length > 0) {
        setLibroDiarioParaReconciliar((prev) => [
          ...prev,
          ...nuevosParaReconciliar,
        ]);
      }
      return data;
    },
  ...
  const addCuentaServicio = useCallback(
    async (consorcioId, servicioId, alias) => {
      const { data, error } = await supabase
      ...
      setConsorcios((prev) =>
        prev.map((c) =>
          c.id === consorcioId
            ? { ...c, serviciosCuentas: [...c.serviciosCuentas, cuenta] }
            : c,
        ),
      );
      return cuenta;
    ...
    setConsorcios((prev) =>
      prev.map((c) =>
        c.id === consorcioId
          ? {
              ...c,
              serviciosCuentas: c.serviciosCuentas.filter(
                (cu) => cu.id !== cuentaId,
              ),
            }
          : c,
  ...
  const addCuentaProveedor = useCallback(
    async (consorcioId, proveedorId, alias) => {
      const { data, error } = await supabase
      ...
      setConsorcios((prev) =>
        prev.map((c) =>
          c.id === consorcioId
            ? { ...c, proveedoresCuentas: [...c.proveedoresCuentas, cuenta] }
            : c,
        ),
      );
      return cuenta;
    ...
    setConsorcios((prev) =>
      prev.map((c) =>
        c.id === consorcioId
          ? {
              ...c,
              proveedoresCuentas: c.proveedoresCuentas.filter(
                (cu) => cu.id !== cuentaId,
              ),
            }
          : c,
  ...
  const addMovimientoManual = useCallback(
    async (consorcioId, tipo, itemId) => {
      let itemNombre = "";
      let mailOrLink = "";
  ...
  const recalcularEstadoPorPagos = useCallback(
    async (movimientoId, pagosDeEsteMovimiento, forzarPagado = false) => {
      ...
      const totalPagado = pagosDeEsteMovimiento.reduce(
      ...
      if (
        nuevoEstado !== movimiento.estado ||
        nuevaFechaPago !== movimiento.fecha_pago
      ) {
        ...
        if (error) throw error;
        setMovimientos((prev) =>
          prev.map((m) => (m.id === movimientoId ? movActualizado : m)),
        );
      }
    },
    [movimientos],
  ...
  const addPagoParcial = useCallback(
    async (
      movimientoId,
      monto,
      fecha,
  ...
  const deletePagoParcial = useCallback(
    async (pagoId, movimientoId) => {
      const { error } = await supabase
        .from("pagos_parciales")
        .delete()
        .eq("id", pagoId);
  ...
  const updateNotaMovimiento = useCallback(
    async (id, notas) => {
  ...
  const generarMes = useCallback(
    async (mesSeleccionado) => {
      const nuevos = [];

      consorcios.forEach((consorcio) => {
        consorcio.serviciosCuentas.forEach((cuenta) => {
          const nombreConAlias = cuenta.alias
            ? `${cuenta.nombre} - ${cuenta.alias}`
            : cuenta.nombre;
