...
function toggleSort(campo) {
  ...
  const movimientosOrdenados = [...movimientos].sort(
    (a, b) =>
      ...
      (a.orden_original || 0) - (b.orden_original || 0),
  );
  ...
  const coincide =
    finalNum !== null && Math.abs(saldoCalculado - finalNum) < 0.01;
  const ultimaFecha =
    movimientosOrdenados.length > 0
      ? movimientosOrdenados[movimientosOrdenados.length - 1].fecha
      : null;
...
const filasParaMostrar = [...filasVisibles].sort((a, b) => {
  ...
  async function guardarSaldos() {
    ...
    try {
      await onUpdatePeriodo(periodo.id, {
        saldo_inicial_declarado:
          saldoInicial === "" ? null : Number(saldoInicial),
  ...
  async function eliminarPeriodo() {
    if (
      !confirm(
        "¿Eliminar este período completo? Se van a borrar también todos sus movimientos. Esta acción no se puede deshacer.",
      )
    )
      return;
    try {
    ...
    <section className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          >
            <i
              className={`fa-solid ${periodo.cerrado ? "fa-lock-open" : "fa-lock"} mr-1`}
            ></i>
            {guardandoCierre
              ? "Guardando..."
              : periodo.cerrado
                  ...
                  return (
                    ...
                    >
                      <p className="text-[10px] text-slate-400 uppercase font-semibold flex items-center gap-1">
                        {d.label}{" "}
                        <span className="text-slate-300">({d.cantidad})</span>
                        {d.inusualesCount > 0 && (
                      ...
                      {d.inusualesCount > 0 && (
                        <p className="text-[10px] text-amber-600 font-semibold mt-0.5">
                          {d.inusualesCount} inusual
                          {d.inusualesCount === 1 ? "" : "es"}
                        </p>
                      )}
                    </button>
                  );
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center flex-wrap gap-2">
          <h3 className="font-bold text-sm text-slate-700">
            ...
            {categoriaFiltro ? ` de ${movimientos.length}` : ""})
            {categoriaFiltro && (
          ...
          <div className="flex gap-2">
            ...
            >
              <i className="fa-solid fa-file-import mr-1"></i> Importar del
              banco
            </button>
            <button
    ...
    <th className="p-3 text-center">Acciones</th>
              ...
              {filasParaMostrar.map((m) => (
                ...
                >
                  ...
                  <td className="p-3 text-right font-mono text-slate-700">
                    {formatMonto(m.saldoCorridoCalculado)}
                  </td>
      ...
      {movimientoEditando && (
        <EditarMovimientoLibroModal
          ...
          servicios={servicios}
          unidades={unidades}
          onGuardar={(movId, campos) =>
            onUpdateMovimiento(periodo.id, movId, campos)
          }
