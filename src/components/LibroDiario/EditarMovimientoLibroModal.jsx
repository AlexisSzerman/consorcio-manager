...
export default function EditarMovimientoLibroModal({ movimiento, proveedores, unidades, onGuardar, onClose }) {
  ...
  const [textoLibre, setTextoLibre] = useState(movimiento.texto_original_banco || '');
  ...
  async function guardar() {
    ...
    try {
      await onGuardar(movimiento.id, {
        ...
        texto_original_banco: categoria === 'sin_clasificar' ? textoLibre.trim() || null : movimiento.texto_original_banco,
        confirmado: true,
      });
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex justify-between items-start">
          <h3 className="font-bold text-slate-900 text-lg">Editar movimiento</h3>
        ...
        {movimiento.texto_original_banco && (
          <p className="text-xs text-slate-400 bg-slate-50 rounded-lg p-2">
            Nombre original del banco: <span className="font-semibold">{movimiento.texto_original_banco}</span>
          </p>
        )}
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            onClick={guardar}
            disabled={guardando || !fecha || !detalle.trim() || !monto}
